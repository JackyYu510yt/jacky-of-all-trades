...
14. **Structural by default — no band-aids unless the user explicitly asked for one.** When designing a fix or a mechanism, the deliverable is the version that removes the failure condition, never the version that only works while conditions happen to be favorable. The signature band-aid shape: a mechanism needs a precondition (logged-in, warmed-up, non-empty, built) and the "fix" **selects inputs that already satisfy it** instead of **establishing it** on whatever input arrives — that works exactly until the pre-qualified pool runs dry (cohort incident 2026-08-20: "promote a spare that's already logged in" stalls the cohort at 3/4 the day no logged-in spare exists; the structural version logs in + clean-slates ANY account as part of promotion, so every spare is promotable). Rule of thumb: **establish, don't filter.** Before shipping any fix, ask "what happens when the favorable subset is exhausted?" — if the answer is "same failure", climb a layer. The only sanctioned band-aid is the work-once-to-smoke-test scaffold in #10, and it is never the deliverable. If the user explicitly requests a quick/temporary patch, ship it labeled `BAND-AID (user-requested):` with the structural version named beside it so the debt is visible.
