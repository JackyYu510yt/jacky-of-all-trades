...
The runbook is the contract; the **board** is the human-readable view of it.
...
It is the one artifact the user reads to know where a long run is without
opening anything.
...
### Promote keeper findings to SPEC.md — and CREATE one when none exists
...
```
Record: SPEC.md updated — <n> finding(s) logged
Record: SPEC.md created + <n> finding(s) logged   (first run in this project)
...
Record:      <what survives this run. "SPEC.md updated — <n> finding(s) logged" | "SPEC.md created + <n> finding(s) logged" | "none — <explicit reason>". MANDATORY on every terminal verdict — a report without this line is malformed. Never omit it to stay quiet; if the scaffold failed, name the path and the error here.>
...
Record:  <"SPEC.md updated — <n> finding(s)" | "SPEC.md created + <n> finding(s)" | "none — <explicit reason>". MANDATORY — never omitted, never silent. Scaffold failed? name path + error here.>
Current stage:
