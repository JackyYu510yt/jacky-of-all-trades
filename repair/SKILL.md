...
16. **Lock the *structural* cause, not the proximate trigger.** "Function returned None on this input" is a trigger; "the function has no validation for empty payloads, and three callers can produce empty payloads" is the structural cause. Step 3 (Lock the cause) is not satisfied until you've climbed one layer up and asked: *"If I fix only this exact line, does the condition that produced this line still exist?"* If yes, the cause is the layer above — keep climbing until "no." The fix targets that layer. A repair that locks the trigger and edits only the trigger is, by definition, symptomatic — and the next workflow run will hit the same failure mode on a different instance. That is not DONE. And when the fix itself adds recovery/handling logic, shape it per **Heaven's Net** — keyed to the evidence-mapped failure class the trigger belongs to, never to the observed symptom string, and sized in PROPORTION: a cooldown / bench / pool pull comes from an observed measurement or a bounded smallest-first ladder, never a constant picked by feel — an under-sized rest is the symptomatic fix in another costume; no measurement → the ladder, measuring it is a follow-up like the refactor (canonical definition + strict evidence guardrails incl. Proportion: /error-recon, "Heaven's Net" section) — within HI #12's blast-radius bound: a refactor to class-level recovery is its own follow-up task after the repair, never a mid-repair expansion.
...
```
 ...
 8. Step 2           → fix holds under real conditions, INCLUDING
                       ...
                       climb another layer.
 9. Audit            → DONE / PARTIAL / STUCK verdict — EXECUTED BY
                       FnReview (added 2026-08-22): a fresh-context
                       reviewer, not the brain that wrote the fix,
                       reads the changed function(s) from disk with
                       the fix packet (failure signature, the step-2
                       hypothesis list + step-3 Lock evidence,
                       rejected approaches) and returns, per item
                       with a file:line citation: the 5 principles
                       (Heaven's Net / evidence-only / re-entry
                       hygiene / no silenced failures / KISS) + the
                       4 completeness tests — does the condition that
                       produced the failure still exist (HI #16)?
                       next run, different input, nobody watching
                       (HI #17)? establish-vs-filter? does the fix
                       target the CONFIRMED cause or route around the
                       symptom (HI #18)? BAND-AID / VIOLATION → not
                       DONE: loop back to step 2/3 as the finding
                       warrants (a weak list → step 2; a wrong lock →
                       step 3), finding = new evidence; bounded per
                       /auto FnReview (fresh approach budget, ≤2 review
                       rounds). Round 2 still BAND-AID → plain /repair
                       ends PARTIAL with the finding under `Still off:`;
                       under /auto the PRODUCING step (7, Integrate)
                       carries PARKED, step 9 parks as its dependent,
                       and the open finding is a DONE gate. Clean → the
                       verdict stands. TIMING UNDER /auto is /auto's
                       FnReview rule, never an extra dispatch: on a
                       /prep-derived runbook the function's AUDIT step
                       is the single executor (this step 9 does NOT
                       dispatch separately — Mode B sub-loops included;
                       if that AUDIT step is already DONE the rewrite
                       re-opens it and the review fires there; a
                       function with no AUDIT step gets the review at
                       the writing step's verify); on a /repair-derived
                       runbook (`/auto /repair`) step 9 IS the executor
                       and the triggers are suppressed at steps 4–8
                       (the step-4 standalone holds a copy of the
                       unfixed def — standalone-harness defs are never
                       reviewed). Plain /repair (no /auto): dispatch a
                       fresh Agent subagent with the canonical brief;
                       subagents unavailable → same-context skeptic
                       pass, marked as the weaker fallback. Canonical
                       mechanism, brief, bounds and stamps: /auto
                       SKILL.md, "FnReview — per-function completeness
                       review" — pointer, not a copy.
```

Every step is gated. If a step fails, loop back to the previous one — do not advance. The user only intervenes when a step refuses to clear after bounded effort (STUCK) or when a Hard Invariant trips. Step 9 is the one step the repairer does NOT grade itself — an independent reviewer does (self-preference bias bites hardest exactly when the fix is weakest).
...
- FnReview still fires if the fix changed a def body (a one-line fix inside a function is still a function-level fix — the reviewer answers "cause removed, or symptom hidden?" in seconds); a fix that touches no def (a constant, a config value, an import line) is exempt.
- Report.
...
FnReview (step 9):        <clean sha:<8> @<ISO> [(same-context fallback)] |
                            WAIVED (<citation>) | unreviewed (<reason>) —
                            a BAND-AID/VIOLATION verdict means this
                            report is not DONE>
...
- **Step 9 is graded by someone else.** A fresh-context FnReview reads the fixed function(s) with the fix packet and rules principles + completeness (cause still exists? next run without Claude? establish-vs-filter? confirmed cause or routed around?); BAND-AID = not DONE, back to step 2/3. Canonical: /auto "FnReview".
