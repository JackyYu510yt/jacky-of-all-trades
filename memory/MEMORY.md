...
- [Goal-compass + confidence/risk footer on reports](feedback_confidence_risk_footer.md) — /explain, /auto, /prep, /spec reports end with ULTIMATE GOAL / NEXT STEP / SUGGESTED ACTION + CONFIDENCE (PERFECT tier = all angles tested, run-unattended grade) + RISK; anything pending caps confidence below HIGH
- [RED-TEAM scenario attack](feedback_redteam_scenario_attack.md) — dedicated attacker subagent, 10 hostile-scenario categories, HANDLED/DEGRADES/BREAKS/UNKNOWN; canonical brief in /audit Phase 5.5, wired into /prep (parallel dispatch), /auto (RedTeam runbook field + rider that fires on unattended/stateful nature even when refuter skipped), /spec (success-criteria sweep)
- [Discriminating tests, not confirming tests](feedback_discriminating_tests.md) — a probe must distinguish rival hypotheses, not confirm the favorite; no premature convergence, no search-space neglect, no anchoring bias; alternatives investigated or explicitly ruled out before DONE. Codified in /repair HI #18 + /auto HI #13 (cite qualified — heuristic #13 differs) + /error-recon Rule 6 + /spec (evidence discipline + quality bar) + /prep (P11 bullet + AUDITOR brief item 5)
- [Guardian trigger gap + first field run](project_guardian_trigger_gap.md) — pasted /auto never loads the skill; global CLAUDE.md rule forces Skill(auto); field predictions P1/P2 confirmed
- [Heaven's Net — class-level recovery](feedback_heavens_net.md) — trigger term: recover by evidence-mapped failure class + required state, never symptom-specific handlers; STRICT evidence-only; canonical in /error-recon, wired into /auto /prep /spec /repair + global CLAUDE.md trigger rule
