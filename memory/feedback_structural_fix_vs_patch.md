...
modified: 2026-08-20T15:43:43.685Z
---
...
**No clash with graduated scale-up (added 2026-08-20, user asked how they coexist):** [[feedback_graduated_scale_up]]'s "build small, progressively scale" looks like it requires band-aids — it doesn't. The two rules turn different dials: **scale-up shrinks SCOPE (how much you run); this rule fixes STRUCTURE (what you build).** Rung 1 of the ramp is the RIGHT fix run small — the real mechanism exercised on one input — never a weaker stand-in mechanism. Cohort example, both dials visible: rung 1 = run the real "log in + clean-slate on promotion" on ONE account (scale-up, correct); "promote an already-logged-in spare" is the wrong mechanism even on all 100 accounts (band-aid — scale doesn't cure structure). Litmus question at rung 1: *"is this a small version of the right mechanism, or a different mechanism that only handles the easy case?"* — first is scale-up, second is a band-aid wearing a scale-up costume. The only legitimate overlap is the scaffold (carve-out above): a work-once hack whose sole job is making a probe/test possible (fake a capped marker, hardcode one input, bypass a login to reach the part under test) — patch to learn, never ship, dead before DONE. Full sequence: **scaffold to probe → build the right fix → prove it small → scale it up.** Pinned in /auto HI #14.
