...
**How to apply:**
- Spread DECIDED 2026-08-22 (user: "lets add it to the other skills"): /prep Phase 8 AUDIT is executed by FnReview (+ fixed SAFE fns), /repair step 9 Audit is executed by FnReview with the fix packet (+ `FnReview (step 9):` line in the DONE report) — both pointer-shaped to /auto's canonical section; backups `prep/SKILL.md.bak-pre-fnreview-*`, `repair/SKILL.md.bak-pre-fnreview-*`. Option B DECIDED 2026-08-22 (user: "I'd rather be super sure than go back and fix stuff"): EVERY function is reviewed (AUTHOR + INLINE), one reviewer per function fanned out in parallel within a step (cap ~8–12), pending reviews don't block steps that don't consume the reviewed function. Only remaining residual: single-reviewer rubber-stamp (accepted). User preference worth remembering: when subagents can run in parallel, they prefer full coverage over saved dispatches — certainty beats cost.
